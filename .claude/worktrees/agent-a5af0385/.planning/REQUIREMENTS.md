...
- [ ] **FOUND-01**: Cấu trúc file tách biệt: `index.html`, `css/main.css`, `css/layout.css`, `css/components.css`, `css/animations.css`, `js/nav.js`, `js/calculator.js`, `js/animations.js`, `js/form.js`
- [ ] **FOUND-02**: `<html lang="vi">`, semantic HTML5 (`<main>`, `<section aria-labelledby>`), tất cả `<script>` dùng `defer`
- [ ] **FOUND-03**: CSS Custom Properties cho toàn bộ design system: màu navy/gold/white, typography, spacing, shadows
- [ ] **FOUND-04**: Google Fonts: Be Vietnam Pro (400, 600) + Montserrat (800) — `subset=vietnamese`, `display=swap`, `preconnect` *(weights updated per UI-SPEC revision 2026-03-28)*
- [ ] **FOUND-05**: Phosphor Icons CSS variant qua CDN
- [ ] **FOUND-06**: `netlify.toml` với security headers (`X-Frame-Options`, `X-Content-Type-Options`, `X-XSS-Protection`)
- [ ] **FOUND-07**: Responsive hoàn toàn: mobile-first, kiểm tra tại 375px, 768px, 1200px
...
- [ ] **CALC-01**: 4 inputs: Giá trị bất động sản, % vay (LTV), Thời hạn (năm), Lãi suất (%/năm)
- [ ] **CALC-02**: Live calculation trên mỗi `input` event — công thức amortization chuẩn: `M = P*[r(1+r)^n]/[(1+r)^n-1]`
- [ ] **CALC-03**: Kết quả hiển thị trả góp hàng tháng định dạng VND (`Intl.NumberFormat('vi-VN', {style:'currency', currency:'VND'})`)
- [ ] **CALC-04**: Kết quả calculator tự động điền vào trường "Số tiền vay" ở form đăng ký
- [ ] **CALC-05**: Tất cả `<input>` trong calculator có `font-size: 16px` (ngăn iOS Safari auto-zoom)
- [ ] **CALC-06**: Disclaimer "Lãi suất tham khảo, có thể thay đổi theo điều kiện thực tế" hiển thị dưới kết quả
...
- [ ] **FORM-01**: 4 fields: Họ tên, Số điện thoại (required, type="tel"), Dự án quan tâm, Số tiền vay (pre-filled từ calculator)
- [ ] **FORM-02**: `font-size: 16px` trên tất cả `<input>` và `<select>` (iOS Safari anti-zoom)
- [ ] **FORM-03**: Validation khi blur: highlight lỗi inline dưới field; scroll đến lỗi đầu tiên khi submit
- [ ] **FORM-04**: Validate số điện thoại Việt Nam: regex `/^(0[35789])\d{8}$/`
- [ ] **FORM-05**: Consent checkbox (mặc định **không tick**, required) — "Tôi đồng ý cho [tên tổ chức] thu thập và xử lý thông tin cá nhân theo Chính sách bảo mật" (Nghị định 13/2023/NĐ-CP)
- [ ] **FORM-06**: Submit button text: "Đăng Ký Tư Vấn Miễn Phí"
- [ ] **FORM-07**: Privacy micro-copy dưới nút submit: "Thông tin được bảo mật tuyệt đối"
- [ ] **FORM-08**: Inline success state sau submit — thay form bằng thông báo "Cảm ơn! Chúng tôi sẽ liên hệ trong 24h" (không redirect)
- [ ] **FORM-09**: Optimistic UI — hiển thị success ngay, không chờ response từ GAS (do CORS `mode:'no-cors'`)
- [ ] **FORM-10**: Form xuất hiện cả ở hero section (dưới CTA) hoặc nút scroll-to-form — không chỉ ở cuối trang
